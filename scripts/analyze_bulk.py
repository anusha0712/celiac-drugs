#!/usr/bin/env python3
"""
analyze_bulk.py — Generate analysis/bulk_findings.md from the flagged bulk dataset.

Reads data/bulk/dailymed_bulk_flagged.csv (72,358 rows) and writes
analysis/bulk_findings.md in one deterministic pass. Every table in the
findings document is computed from the same in-memory dataframe. No hand
transcription. Rerunning this script produces a byte-identical file.
...
Run:
  python3 scripts/analyze_bulk.py
"""
...
INPUT_CSV = BASE_DIR / "data" / "bulk" / "dailymed_bulk_flagged.csv"
...
*Input file: `data/bulk/dailymed_bulk_flagged.csv` (72,358 rows)*
...
documented in `analysis/limitations.md` Part 4.
...
Full limitations: `analysis/limitations.md` Part 4.
"""
...
`analysis/limitations.md` § 2.1 on the label-only methodology.
...
`analysis/limitations.md` § 2.7 and § 4.5.
...
def section_pilot_comparison(df):
    ...
    # Pilot numbers from analysis/findings.md
    rows = [
        (
...
The pilot dataset (`data/eda/dailymed_flagged.csv`, 6,605 products) covered
...
stricter filter pipeline (`scripts/filter_bulk.py`) that additionally drops
...
Pilot percentages are taken from `analysis/findings.md` § 4.1 and § 4.2.
