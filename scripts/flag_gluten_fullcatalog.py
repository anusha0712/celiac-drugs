#!/usr/bin/env python3
"""
flag_gluten_fullcatalog.py — Apply gluten flags to the filtered bulk DailyMed dataset.
...
Input:
  data/fullcatalog/gluten_fullcatalog_filtered.csv     (72,358 rows × 43 columns)
  data/fullcatalog/gluten_validation.csv       (171 entries: 105 gluten_free, 35 unknown, 31 contains_gluten)

Output:
  data/fullcatalog/gluten_fullcatalog_flagged.csv      (72,358 rows × 45 columns; adds gluten_flag, flagged_excipients)
...
INPUT_CSV = BASE_DIR / "data" / "fullcatalog" / "gluten_fullcatalog_filtered.csv"
VALIDATION_CSV = BASE_DIR / "data" / "fullcatalog" / "gluten_validation.csv"
OUTPUT_CSV = BASE_DIR / "data" / "fullcatalog" / "gluten_fullcatalog_flagged.csv"

# Gluten validation table is exact-match-only (no regex, no match_type
# column). No try/except for malformed regex needed here — that guard
# lives in flag_allergens_fullcatalog.load_validation() per Task #35.
...
def main():
    if not INPUT_CSV.exists():
        ...
        print("Run scripts/filter_gluten_fullcatalog.py first.", file=sys.stderr)
        sys.exit(1)
